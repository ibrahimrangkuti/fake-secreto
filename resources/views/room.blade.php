    ...
    <div class="flex justify-center p-8">
        <div class="w-96 bg-white p-6 rounded-lg animate__animated animate__swing">
            ...
            <form action="{{ route('room.send-message', $room->slug) }}" method="POST">
                @csrf
                <input type="text"  name="message" class="w-full mt-3 mb-4 py-2 px-4 ring-1 ring-slate-500 rounded-lg text-[#7743DB] focus:outline-none focus:ring-1 focus:ring-[#7743DB] placeholder:text-slate-500 focus:placeholder:text-[#7743DB] placeholder:text-sm" placeholder="Kasih pesan buat ...">
